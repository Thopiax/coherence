defmodule CoherenceTest.TrackableService do
  ...
  describe "Trackable" do
    test "first login", %{conn: conn, user: user} do
      ...
      assert utc_eq?(current_user.last_sign_in_at, current_user.current_sign_in_at)
    ...
    test "2nd login", %{conn: conn, user: user} do
      ...
      assert current_user.current_sign_in_at
      assert current_user.current_sign_in_ip == "{127, 0, 0, 1}"
      refute utc_eq?(current_user.last_sign_in_at, current_user.current_sign_in_at)
      assert current_user.last_sign_in_ip == current_user.current_sign_in_ip
    end
    ...
    test "track_logout", %{conn: conn, user: user} do
      ...
      assert utc_eq?(current_user(conn).current_sign_in_at, user1.last_sign_in_at)
    end
  end
  describe "Trackable-Table" do
    test "first login", %{conn: conn, user: user} do
      ...
      assert utc_eq?(trackable.last_sign_in_at, trackable.current_sign_in_at)
    ...
    test "track_logout", %{conn: conn, user: user} do
      ...
      assert utc_eq?(t1.last_sign_in_at, t2.last_sign_in_at)
    end
    test "second login", %{conn: conn, user: user} do
      ...
      assert utc_eq?(t2.last_sign_in_at, t3.last_sign_in_at)
  ...
  defp utc_eq?(%DateTime{} = dt1, %DateTime{} = dt2) do
    remove_microsecond(dt1) == remove_microsecond(dt2)
  end

  defp remove_microsecond(%DateTime{} = dt), do: struct(dt, microsecond: {0, 0})
end
